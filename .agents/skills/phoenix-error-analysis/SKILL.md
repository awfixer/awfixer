...
- **Session** — a sequence of traces sharing a `session.id`. Multi-turn agents where the failure is a _trajectory_ — context loss across turns, goal drift, forgotten preferences — that exists only _across_ traces.
...
3. **Turn structure.** Open one recent trace's root-span `input.value` (it may be large — filter rather than reading wholesale). A single user message → one shot; a message _array_ (`[{role: user}, {role: assistant}, ...]`) → a turn of a dialogue that lives at the session level.
...
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Sample, expand, drill     | `px trace list`, `px trace get <trace-id>`, `px span get <span-id>`, `px session get <session-id>`                                            | `Project.spans(filterCondition: "parent_span is None")`, `getTraceByOtelId`, `getSpanByOtelId`, `node(id:)` on a session                                                                                             |
...
2. **One dimension per annotation.** Judging relevance _and_ faithfulness means two annotations; so does judging tool choice _and_ task completion.
