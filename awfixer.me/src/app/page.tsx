...
export default async function Home() {
  ...
  const navigationSlices = home.data.slices.filter(
    (slice: any) => slice.slice_type === "navigation_menu",
  );
  const contentSlices = home.data.slices.filter(
    (slice: any) =>
      slice.slice_type !== "navigation_menu" && slice.slice_type !== "footer",
  );
  const footerSlices = home.data.slices.filter(
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    description:
      home.data.meta_description || "AWFixer - Personal website and blog",
    openGraph: {
