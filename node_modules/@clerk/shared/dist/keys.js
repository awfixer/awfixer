...
const require_keySetupGuidance = require('./_chunks/keySetupGuidance-fwM5Pphw.js');
...
function parsePublishableKey(key, options = {}) {
	...
	if (!key || !isPublishableKey(key)) {
		if (options.fatal && !key) throw new Error(`Publishable key is missing.\n\n${require_keySetupGuidance.keySetupGuidance}`);
		if (options.fatal && !isPublishableKey(key)) throw new Error(`Publishable key not valid (expected format: pk_test_... or pk_live_...).\n\n${require_keySetupGuidance.keySetupGuidance}`);
		return null;
	}
