//#region src/phone/countryCodeData.ts
const data = [
	[
		"United States",
		"us",
...
const UnitedStatesSubAreasSet = new Set([
	"907",
	"205",
	"251",
	"256",
...
const CanadaSubAreaSet = new Set([
	"204",
	"226",
	"236",
	"249",
...
const SubAreaCodeSets = {
	us: UnitedStatesSubAreasSet,
	ca: CanadaSubAreaSet
};
const createEntry = ([name, iso, code, pattern = "", priority = 0]) => ({
	name,
	iso,
	code,
	pattern,
...
const IsoToCountryMap = data.reduce((acc, cur) => acc.set(cur[1], createEntry(cur)), /* @__PURE__ */ new Map());
const CodeToCountriesMap = data.reduce((acc, cur) => {
	const code = cur[2];
	const country = createEntry(cur);
	if (acc.has(code)) acc.get(code)?.push(country);
	else acc.set(code, [country]);
	return acc;
}, /* @__PURE__ */ new Map());
...
//#endregion
export { CodeToCountriesMap, IsoToCountryMap, SubAreaCodeSets, byPriority, extractDigits, formatPhoneNumber, getCountryFromPhoneString, getCountryIsoFromFormattedNumber, getFlagEmojiFromCountryIso, getPreferredPhoneCodeChannelByCountry, parsePhoneString, stringToFormattedPhoneString };
