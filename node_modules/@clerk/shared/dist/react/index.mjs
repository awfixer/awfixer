...
import { C as isClerkAPIResponseError } from "../_chunks/error-Gl5OMeMr.mjs";
import { logger } from "../logger.mjs";
import { getCurrentOrganizationMembership } from "../organization.mjs";
import { a as eventMethodCalled } from "../_chunks/telemetry-9C6N5ppw.mjs";
...
const INTERNAL_STABLE_KEYS = {
	...
	ORGANIZATION_SSO_BYPASS_ALLOWLIST_KEY
};
...
const HOOK_NAME$5 = "useOAuthConsent";
/**
...
function useOAuthConsent(params) {
	useAssertWrappedByClerkProvider(HOOK_NAME$5);
	...
	clerk.telemetry?.record(eventMethodCalled(HOOK_NAME$5));
...
const HOOK_NAME$4 = "useOAuthDeviceVerification";
const normalizeUserCode = (userCode) => userCode.toUpperCase().replace(/[-\p{White_Space}]/gu, "");
const createNotReadyError = () => new ClerkRuntimeError("Clerk must be loaded before using OAuth device verification.", { code: "oauth_device_verification_not_ready" });
const createLookupInProgressError = () => new ClerkRuntimeError("An OAuth device verification lookup is already in progress for another code.", { code: "oauth_device_verification_lookup_in_progress" });
const createSubmissionInProgressError = () => new ClerkRuntimeError("Another OAuth device verification decision is already in progress.", { code: "oauth_device_verification_submission_in_progress" });
/**
...
function useOAuthDeviceVerification() {
	...
	const lookup = useCallback((params) => {
	...
	const submit = useCallback((params, approved) => {
		const oauthApplication = clerk.loaded ? clerk.oauthApplication : void 0;
		if (!oauthApplication) return Promise.reject(createNotReadyError());
	...
	return {
		...
		isSubmitting,
		lookup,
		approve: useCallback((params) => submit(params, true), [submit]),
		deny: useCallback((params) => submit(params, false), [submit]),
		reset: useCallback(() => {
...
function useOrganizationDomains(params = {}) {
	...
	const unverifiedOwnershipKey = useMemo(() => (response?.data ?? []).filter((domain) => domain.ownershipVerification?.status === "unverified").map((domain) => domain.id), [response?.data]).join(",");
	useEffect(() => {
...
function useOrganizationDirectorySync(params) {
	...
	const revalidate = useCallback(() => queryClient.invalidateQueries({ queryKey: invalidationKey }), [queryClient, invalidationKey]);
	const createDirectorySync = useCallback(async (createParams) => {
	...
	const updateDirectorySync = useCallback(async (updateParams) => {
		if (!directory) return;
...
function useOrganizationDirectorySyncUsers(params) {
	const { directory, params: fetchParams = {
		initialPage: 1,
		pageSize: 10
	}, poll = false, pollIntervalMs = DEFAULT_POLL_INTERVAL_MS$1, enabled = true, keepPreviousData = true } = params;
	const clerk = useClerkInstanceContext();
	const organization = useOrganizationBase();
	const [queryClient] = useClerkQueryClient();
...
function useOrganizationEnterpriseConnectionTestRuns(params) {
	...
	const queryEnabled = enabled && clerk.loaded && Boolean(organization) && Boolean(enterpriseConnectionId);
	const [pollingConnectionId, setPollingConnectionId] = useState(null);
	...
	useEffect(() => {
		if (shouldPoll && hasRows) setPollingConnectionId(null);
	}, [shouldPoll, hasRows]);
	const revalidate = useCallback(async (options) => {
		if ((options?.armPolling ?? true) && !hasRows) setPollingConnectionId(enterpriseConnectionId);
		if (options?.exact) await queryClient.invalidateQueries({
			queryKey,
			exact: true
		});
		else await queryClient.invalidateQueries({ queryKey: invalidationKey });
...
function useOrganizationSSOBypassAllowlist(params = {}) {
	...
	const organization = useOrganizationBase();
	const [queryClient] = useClerkQueryClient();
	const { queryKey, stableKey, authenticated } = useOrganizationSSOBypassAllowlistCacheKeys({ organizationId: organization?.id ?? null });
	const queryEnabled = enabled && clerk.loaded && Boolean(organization);
	useClearQueriesOnSignOut({
		isSignedOut: organization === null,
		authenticated,
	...
	const revalidate = useCallback(() => queryClient.invalidateQueries({ queryKey: [stableKey] }), [queryClient, stableKey]);
	const addUser = useCallback(async (addParams) => {
		const added = await organization?.ssoBypassAllowlist.addUser(addParams);
		await revalidate();
		return added;
	}, [organization, revalidate]);
	const addUsers = useCallback(async (addParams) => {
		try {
...
//#endregion
export { ClerkContextProvider, ClerkInstanceContext, InitialStateProvider, OptionsContext, UNSAFE_PortalProvider, __createClerkTestQueryClient, __experimental_CheckoutProvider, PaymentElement as __experimental_PaymentElement, PaymentElementProvider as __experimental_PaymentElementProvider, useCheckout as __experimental_useCheckout, usePaymentAttempts as __experimental_usePaymentAttempts, usePaymentElement as __experimental_usePaymentElement, usePaymentMethods as __experimental_usePaymentMethods, usePlans as __experimental_usePlans, useStatements as __experimental_useStatements, useSubscription as __experimental_useSubscription, useClientBase as __internal_useClientBase, __internal_useCreditBalanceQuery, __internal_useCreditHistoryQuery, useOrganizationBase as __internal_useOrganizationBase, useOrganizationDirectorySync as __internal_useOrganizationDirectorySync, useOrganizationDirectorySyncUsers as __internal_useOrganizationDirectorySyncUsers, useOrganizationDomains as __internal_useOrganizationDomains, useOrganizationEnterpriseConnectionTestRuns as __internal_useOrganizationEnterpriseConnectionTestRuns, useOrganizationEnterpriseConnections as __internal_useOrganizationEnterpriseConnections, useOrganizationSSOBypassAllowlist as __internal_useOrganizationSSOBypassAllowlist, usePaymentAttemptQuery as __internal_usePaymentAttemptQuery, __internal_usePlanDetailsQuery, useSessionBase as __internal_useSessionBase, useStatementQuery as __internal_useStatementQuery, useUserBase as __internal_useUserBase, useUserEnterpriseConnections as __internal_useUserEnterpriseConnections, __resetClerkQueryClientForTest, __setClerkQueryClientForTest, assertContextExists, createContextAndHook, getClerkQueryClient, isDeeplyEqual, useAPIKeys, useAssertWrappedByClerkProvider, useAttemptToEnableOrganizations, useClerk, useClerkInstanceContext, useDeepEqualMemo, useInitialStateContext, useOAuthConsent, useOAuthDeviceVerification, useOptionsContext, useOrganization, useOrganizationCreationDefaults, useOrganizationList, usePortalRoot, useReverification, useSafeLayoutEffect, useSession, useSessionList, useUser };
