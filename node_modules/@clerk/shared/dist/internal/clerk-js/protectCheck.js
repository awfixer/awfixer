...
require('../../_chunks/error-DusVHo0B.js');
...
function assertValidSdkUrl(sdkUrl) {
	...
	try {
		...
		throw new require_clerkRuntimeError.ClerkRuntimeError("Protect check sdk_url is not a valid URL", { code: "protect_check_invalid_sdk_url" });
	}
	if (parsed.protocol !== "https:") throw new require_clerkRuntimeError.ClerkRuntimeError("Protect check sdk_url must use HTTPS", { code: "protect_check_invalid_sdk_url" });
	if (parsed.username || parsed.password) throw new require_clerkRuntimeError.ClerkRuntimeError("Protect check sdk_url must not contain credentials", { code: "protect_check_invalid_sdk_url" });
	return parsed;
}
/**
...
async function executeProtectCheck(protectCheck, container, options = {}) {
	...
	if (signal?.aborted) throw new require_clerkRuntimeError.ClerkRuntimeError("Protect check aborted by caller", { code: "protect_check_aborted" });
	if (typeof mod.default !== "function") throw new require_clerkRuntimeError.ClerkRuntimeError("Protect check script does not export a default function", { code: "protect_check_invalid_script" });
	let proofToken;
	try {
		...
		if (signal?.aborted && looksLikeAbort) throw new require_clerkRuntimeError.ClerkRuntimeError("Protect check aborted by caller", { code: "protect_check_aborted" });
		throw new require_clerkRuntimeError.ClerkRuntimeError(`Protect check script execution failed: ${err instanceof Error ? err.message : String(err)}`, { code: "protect_check_execution_failed" });
	}
	if (signal?.aborted) throw new require_clerkRuntimeError.ClerkRuntimeError("Protect check aborted by caller", { code: "protect_check_aborted" });
	return proofToken;
}
