  ...
  async createBatch(messages) {
    ...
    return results.map(
      (result) => result.email ? { index: result.index, email: Email.fromJSON(result.email), statusCode: result.status_code } : {
        index: result.index,
        errors: (result.errors || []).map(import_error.parseError),
...
var assertOAuthAudienceClaim = (aud, audience) => {
  const audienceList = [audience].flat().filter((a) => !!a);
  if (audienceList.length === 0) {
    return;
  }
  const audFromToken = aud ? [aud].flat() : void 0;
...
var IdPOAuthAccessTokenApi = class extends AbstractAPI {
  ...
  async verify(accessToken, options = {}) {
    const verifiedToken = await this.request({
      method: "POST",
...
var IdPOAuthAccessToken = class _IdPOAuthAccessToken {
  constructor(id, clientId, type, subject, scopes, revoked, revocationReason, expired, expiration, createdAt, updatedAt, aud) {
    this.id = id;
  ...
  static fromJwtPayload(payload, clockSkewInMs = 5e3) {
    ...
    return new _IdPOAuthAccessToken(
      ...
      oauthPayload.aud === void 0 ? void 0 : [oauthPayload.aud].flat()
    );
  }
};
...
var API_VERSION = "v1";
var USER_AGENT = `${"@clerk/backend"}@${"3.20.0"}`;
...
var errorThrower = (0, import_error4.buildErrorThrower)({ packageName: "@clerk/backend" });
...
function getOAuthAudienceVerificationError(aud, audience) {
  try {
...
function parseErrors(data) {
  if (!!data && typeof data === "object" && "errors" in data) {
    ...
    return errors.length > 0 ? errors.map(import_error5.parseError) : [];
  }
  return [];
}
function withLegacyRequestReturn(cb) {
  return async (...args) => {
    ...
    if (errors) {
      const error = new import_error5.ClerkAPIResponseError(statusText || "", {
        data: [],
        status,
        clerkTraceId,
        retryAfter
...
var import_error6 = require("@clerk/shared/error");
...
function handleClerkAPIError(tokenType, err, notFoundMessage) {
  if ((0, import_error6.isClerkAPIResponseError)(err)) {
    let code;
    let message;
    switch (err.status) {
      case 401:
