...
function parseImports(css) {
	const out = [];
	for (const match of css.matchAll(/@import\s+(?:url\(\s*)?["']([^"']+)["']\s*\)?[^;]*;/g)) out.push(match[1]);
	return out;
}
...
function exportRoute(module, file, name) {
	...
	for (;;) {
		...
		if (aliased) continue;
		if (name === "default" && module.defaultName) return importedRoute(module.defaultName);
...
function componentFiles(dir) {
	...
	for (const entry of entries) {
		...
		else if (entry.isDirectory()) for (const index of [
			"index.tsx",
			"index.ts",
			"index.jsx"
		]) {
...
function build(file, patterns, visiting, deps, parsedAst) {
	...
	if (!ast) {
		...
		if (!source.includes("className") && !source.includes("...")) return {
			wrappers,
			complete
		};
		try {
...
function isEscaped(variable, context) {
	return (variable.references ?? []).some((ref) => {
		...
		if (context) {
			...
			if (attribute?.type === "JSXAttribute" && (attribute.name?.name === "style" || isClassAttribute(attributeNameOf(attribute)))) return false;
			let container = value;
			let outer = parent;
...
function unwrapTs(node) {
	while (node && (node.type === "TSAsExpression" || node.type === "TSNonNullExpression" || node.type === "TSSatisfiesExpression" || node.type === "JSXExpressionContainer")) node = node.expression;
	return node;
}
...
function renderedOpeningOf(element) {
	const expression = (element.attributes ?? []).find((candidate) => candidate.type === "JSXAttribute" && candidate.name?.name === "render")?.value?.expression;
...
function classSiteVisitors(context, options, onSite) {
	...
	const siteFor = (node, value, attribute, resolved, element = null) => {
		return {
			...value ? collectFromValue(value, context, collectOptions) : collectClassStrings(node, context, collectOptions),
	...
	const attributeSites = (node) => {
		...
		if (list) return list;
		list = [siteFor(node, node.value, attributeNameOf(node), elementOf(node), jsxElementOf(node))];
		sites.set(node, list);
		return list;
	};
	const spreadSites = (node) => {
		...
		if (list) return list;
		list = [];
		const object = resolveObject(node.argument, context, /* @__PURE__ */ new Set());
		if (object) {
...
const noInlineStyles = {
	...
	create(context) {
		...
		const elementName = (attribute) => {
			...
			if (name?.type === "JSXIdentifier") return /^[A-Z]/.test(name.name) ? name.name : "";
			if (name?.type === "JSXMemberExpression") {
				const parts = [];
				for (let n = name; n; n = n.object) if (n.type === "JSXMemberExpression") parts.unshift(n.property.name);
		...
		const check = (expr, reportAt, seen = /* @__PURE__ */ new Set(), component = "") => {
			...
			for (const [key, value] of properties) {
				...
				else if (carriesRawColor(value, context)) emit({
					node: value,
		...
		return {
			...
			JSXAttribute(node) {
				...
				check(value.expression, value.expression, /* @__PURE__ */ new Set(), elementName(node));
			},
			JSXSpreadAttribute(node) {
				const object = resolveObject(node.argument, context, /* @__PURE__ */ new Set());
				if (!object) return;
...
function workerFile() {
	...
	if (/\/src\/tailwind\/[^/]+$/.test(import.meta.url)) candidates.push(new URL("../../dist/tailwind-worker.js", import.meta.url));
